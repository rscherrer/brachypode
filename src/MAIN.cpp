...
// Lambda for identifying dead individuals
...
void cleanup(std::vector<Individual> &pop) {

    // pop: the population

    // Prepare to remove dead individuals
...
void doMain(const std::vector<std::string> &args) {
    ...
    // TODO: Consistent punctuation in error messages
    ...
    auto arch = std::make_shared<Architecture>(pars.nchrom, pars.nloci, pars.effect);
    ...
    if (pars.loadarch) {
        ...
        pars.nchrom = arch->nchrom;
        pars.nloci = arch->nloci;

        // Check
        assert(pars.nchrom == arch->nchrom);
        assert(pars.nloci == arch->nloci);
    ...
    for (size_t i = 0u; i < pars.popsize; ++i) 
        pop.emplace_back(Individual(pars.allfreq, arch));
    ...
    for (size_t t = 0u; t <= pars.tend; ++t) {
        ...
        for (size_t i = 0u; i < pop.size(); ++i) {
            ...
            const double tol = pop[i].getTolerance();
            ...
            if (timetosave && buffers["individuals"]) {
                ...
                buffers["individuals"]->save(deme_);
                buffers["individuals"]->save(patch_);
                buffers["individuals"]->save(tol);
        ...
        if (timetosave && buffers["patchsizes"]) {
            ...
            for (size_t j = 0u; j < patchsizes.size(); ++j) {
                ...
                // Save patch size
                buffers["patchsizes"]->save(patchsize_);
        ...
        for (size_t j = 0u; j < patchsizes.size(); ++j) {
            ...
            if (timetosave && buffers["traitmeans"]) buffers["traitmeans"]->save(meanx[j]);
        ...
        for (size_t i = 0u; i < nadults; ++i) {
            ...
            double r = pars.maxgrowth - pars.tradeoff * tol;

            // Useful exponent that might be needed for non-linear trade-offs
            const double d = -log2(1.0 - pars.tradeoff);

            // Modify if needed
            if (pars.type == 2u) r = pars.maxgrowth * pow(1.0 - pow(tol / arch->tolmax, 1.0 / d), d);
            ...
            for (size_t j = 0u; j < nseeds; ++j) {
                ...
                if (rnd::bernoulli(1.0 - pars.selfing)(rnd::rng)) {
                    ...
                    pop.back().recombine(pars.recombination, pop[k]);
                ...
                const double xoff = pop.back().getTolerance();
